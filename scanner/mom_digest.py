    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "watchlist_zh": {
                ...
                "description": "3-5只A股/港股/ETF。每只都必须由今天的某个事件或板块催化、并给出多/空方向。优先板块龙头或行业ETF（最干净的表达），不要每次都堆同样的大众名。重要：previously_recommended 里仍在跟踪、催化逻辑未失效的标的应当**保留**（沿用其方向），不要无故换名；只在出现新催化或旧逻辑失效时才增删或调向。benched 列表中的标的为近期连续跑输、已被系统停用项，禁止再次同向推荐（会被硬过滤丢弃）。",
            },
...
- **已停用名单（benched，强约束）**：输入里的 benched 列出**近期连续多个周期都跑输、已被系统停用**的标的（如 3 日、5 日盯市均为负）。本次**绝对不要**再以相同方向推荐它们，也不要靠换名或换同板块的等价标的变相保留同一笔判断——系统会在输出端再做一次硬过滤，重复推荐只会被直接丢弃、白白浪费名额。只有当某标的出现与原方向**相反**的全新催化时，才可作为一笔新判断重新表达。宁可少给几只，也不要凑数。
...
def _format_user(
    ...
    if watchlist_ctx:
        ...
        # Sustained losers the tracker has benched — Opus must not re-recommend these.
        if watchlist_ctx.get("benched"):
            payload["benched"] = watchlist_ctx["benched"]
...
def _format_benched(benched: list[dict] | None) -> str:
    """One quiet line naming the picks the tracker just suppressed for 连续跑输, so a
    name vanishing from 建议关注 reads as a deliberate bench, not an oversight."""
    if not benched:
        return ""
    parts = []
    for b in benched:
        rr = b.get("running_return_pct")
        parts.append(f"{b.get('name_zh', '')}（{rr:+.1f}%）" if rr is not None else f"{b.get('name_zh', '')}")
    return "\n🛑 _近期连续跑输已停用：" + "、".join(parts) + "_"
...
def run(
    ...
    # Deterministic backstop: drop any pick the tracker has benched for sustained
    # underperformance (连续跑输), no matter what Opus returned. Keeps names like
    # 紫金矿业 off the card once they've lost across consecutive horizons.
    now = datetime.now(timezone.utc)
    raw_watchlist, benched_out = mom_watchlist.filter_benched(raw_watchlist, now)
    if benched_out:
        log.info(
            "Mom digest: suppressed %d benched pick(s): %s",
            len(benched_out),
            ", ".join(
                f"{d.get('name_zh')}({d.get('code')} {d.get('direction')} "
                f"{d.get('running_return_pct')}%)" for d in benched_out
            ),
        )
    preview = mom_watchlist.preview(raw_watchlist)
    card = _build_card(result, preview, watchlist_ctx.get("track_record"), benched_out)
    ...
    if ok:
        ...
        mom_watchlist.commit(raw_watchlist, result.get("confidence"), now)
